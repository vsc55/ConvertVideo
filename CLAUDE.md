...
- **Manual (`manual\`).** Guía de USO con capturas (`manual\README.md` + `01…04`): qué ves en cada ventana y qué hacer. No duplica `docs\` — enlaza. Las imágenes de `manual\img\` **no se recortan a mano**: las regenera `manual\generar-capturas.ps1` (`-Sta`; abre las ventanas de verdad sobre un root temporal sembrado con las fixtures de `test\` renombradas, y `-Only cola,job,preparar,setup` limita el grupo). Si cambias una ventana, regenera su grupo y mira el PNG.

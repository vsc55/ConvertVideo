...
| Columna | Qué muestra |
|---|---|
| **Bordes** | `[x]` se han detectado barras y se recortan · `[ ]` se buscaron y no había · *(vacío)* no se buscó, o no se puede saber. En un archivo **ya convertido** no hay job (el worker lo borra al terminar), así que se **deduce**: recortar cambia la proporción y reescalar la conserva, de modo que comparando el tamaño del original con el de la salida se sabe si se quitaron barras (`Get-CvBorderFromSizes`, pura y con tests). Eso cuesta dos `ffprobe` por archivo (~300 ms medidos), y no por el tamaño del archivo —leer un mkv de 1 GB cuesta lo mismo que uno de 3 MB, 99 ms: lo que se paga es **arrancar** un ejecutable de 231 MB—, así que se analizan unos pocos por refresco —`gui.queueDoneProbe`, 2 de serie— y **solo con la cola parada**: mientras hay workers, el refresco tiene que seguir siendo barato. Lo analizado se cachea **en disco** —en `<config>.gui.json`, junto a cómo quedó la ventana—, así que solo se calcula la primera vez: al volver a abrir la cola la columna sale rellena de entrada (medido: 1.701 ms la primera vez, **116 ms** la siguiente). La huella es el tamaño y la fecha de los **dos** ficheros, de modo que si cambia alguno se vuelve a mirar; y al cerrar la ventana se tiran las entradas de archivos que ya no existen, para que la caché no acumule restos. Se puede vaciar a mano desde *setup → Mantenimiento*. |

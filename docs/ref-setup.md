...
| **Mantenimiento** · Limpiar (jobs, bloqueos, temporales, logs y cachés) | **Todo lo que se puede limpiar, en una lista con su recuento**: jobs, bloqueos y temporales de `Proceso\`, logs de sesiones anteriores (el de la sesión en curso **no** se toca) y lo cacheado por las ventanas (cómo quedó cada una y los archivos ya analizados). Se marca lo que se quiere tirar, se ve cuánto se va a borrar —con aviso si entran los **jobs**, que obligan a volver a preparar— y se limpia de una vez. Antes cada cosa tenía su botón y había que ir a buscarlas. Datos y borrado: `Get-CvSetupMaintenanceItems` / `Invoke-CvSetupMaintenance`, los mismos en las dos caras. |
| **Mantenimiento** · Limpiar Proceso fichero a fichero | Borra `*.job.json`, los ficheros de **control de los workers** (`*.lock`, `<pid>.worker.json`, `stop.flag` — ver [ref-cola.md](ref-cola.md)), temporales o todo (con confirmación); las cuentas se muestran en el menú. Patrones: `Get-CvProcesoPatterns`. |
| **Logs** · Ver logs | Abre el visor de `logs\` (borrar logs está en *Mantenimiento*). |
| Salir | — |

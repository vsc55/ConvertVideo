...
| `video.keepOriginalIfBigger` | `false` | **Si recodificar engorda el vídeo, quedarse con la pista original.** Al terminar de codificar se compara lo que ha salido con la pista del original: si ocupa **más**, recodificar no ha compensado, así que se tira lo codificado y se multiplexa el **vídeo original** — el audio, los subtítulos y los capítulos ya están hechos y se conservan tal cual, no se repite nada. Solo se hace cuando la imagen **no se toca**: sin recorte, sin escalado, sin tone-mapping HDR→SDR y con el mismo fps de salida que el de origen (si la imagen cambia, el original ya no sirve de sustituto). Lo que se mira es si **cambia de verdad**, no si el ajuste está puesto: un escalado que deja el mismo tamaño (`ChangeSize 1920:-2` sobre un vídeo que ya es de 1920) o un recorte que abarca el fotograma entero **no** cuentan, igual que forzar un fps que ya es el del origen — si contaran, la opción no se aplicaría casi nunca. Cuando algo sí cambia, el log lo dice: *«No se mira si compensa: la imagen cambia (escalado 1280:-2)…»*. El tamaño de la pista original se saca sin leer el fichero entero (tag `NUMBER_OF_BYTES` de mkvmerge → `bit_rate` × duración → tag `BPS` → y si no hay nada, el tamaño del archivo como cota superior, que es conservador). Es el valor **de partida**: cada archivo lo lleva congelado en su job (`video.keepOriginal`) y se puede cambiar por archivo. La salida queda **marcada** (`CV_VIDEO=original`) y la cola lo enseña en su fila. Se mira **dos veces**, porque cada comparación ve lo que la otra no: (1) por etapas, la pista codificada contra la del original, antes de multiplexar —lo más fino, pero depende de que se sepa cuánto ocupa la original—; y (2) siempre, con el fichero ya hecho, **la salida contra la entrada**, que no estima nada. En la segunda (y en la beta de una pasada, donde no hay momento intermedio) el cambio se hace con un **remux** que le pone a la salida la pista de vídeo del original, copiando: no se recodifica nada. Por eso una salida puede quedar marcada como *vídeo original* y aun así ser **más pequeña** que la entrada: el vídeo es el de partida y lo que ha adelgazado es el audio. |
| `video.keepOriginalRatio` | `1.00` | Desde qué proporción se da por no compensado. `1.00` = solo si el recodificado es **más grande**; `0.95` = también si no ahorra al menos un 5%. |
...
| `queueRefreshMs` | `1000` | Cada cuánto se repinta el **progreso** mientras hay workers. Solo lee lo que publican ellos (`Proceso\*.worker.json`); las carpetas de archivos no se tocan. |
| `queueIdleRefreshMs` | `0` | Cada cuánto se **releen las carpetas** sin nada en marcha. `0` = nunca: la lista se actualiza con *Actualizar*, con cualquier acción de la ventana, al volver a ella, y cuando un worker cambia de archivo o termina. Súbelo (p. ej. `10000`) si quieres un latido automático. |
| `queueRefreshOnActivate` | `true` | Releer las carpetas al volver a la ventana, para recoger lo que hayas dejado en `Original\`. |
| `queueFollowWorker` | `true` | La lista **se mueve sola** para no perder de vista el archivo que se está codificando cuando su fila se sale de la zona visible. Si el scroll lo has movido tú y has dejado esa fila fuera, se suelta (nada de tirones mientras miras otra parte) y vuelve a engancharse cuando la tienes otra vez delante. |
| `queueStartGraceSec` | `20` | Segundos que la cola sigue mirando tras pulsar *Iniciar*, esperando a que un worker recién abierto **publique su estado** (tarda unos segundos en arrancar). En ese hueco no hay workers vivos, pero sí algo que esperar: sin esto la lista se quedaba parada hasta el siguiente *Actualizar*. También es lo que mantiene *Iniciar* apagado para que no se pulse dos veces. |
| `queueFollowHoldSec` | `3` | Segundos que la lista se queda **quieta** después de que la toques (marcar filas, teclas), además de mientras tengas el ratón apretado. Es lo que evita que un salto de la lista en mitad de un *Ctrl/Mayús+clic* acabe seleccionando lo que no era. |
